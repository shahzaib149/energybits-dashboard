...
function engagementBadge(rate: number): { label: string; class: string } {
  if (rate >= 70) return { label: "Excellent", class: "bg-emerald-500/15 text-emerald-400" };
  if (rate >= 55) return { label: "Good", class: "bg-brand/15 text-brand" };
  return { label: "Fair", class: "bg-amber-500/15 text-amber-400" };
}
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-border/60">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-border/40">
                {visible.map((row, i) => {
                  const badge = engagementBadge(row.engagementRatePct);
